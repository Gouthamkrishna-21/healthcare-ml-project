...
# Custom CSS for "Real Website" Look and Uniform Sizing
st.markdown("""
    <style>
    ...
    /* FIX: Added the missing plot-holder class for uniform chart sizes */
    .plot-holder {
        background-color: white;
        padding: 15px;
        border-radius: 12px;
...
# ==========================================
# 2. LOGIC (Internal Logic Unchanged)
# ==========================================
...
# 4. MAIN INTERFACE
# ==========================================
st.markdown(f'<div class="main-header"><h1>🩺 Clinical Decision Support System</h1><p>HFR-MADM Optimized Analysis | Active Source: {dataset_choice}</p></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    with col1:
        ...
        fig1, ax1 = plt.subplots(figsize=(6, 4)) # Standardized Size
    ...
    with col2:
        ...
        fig2, ax2 = plt.subplots(figsize=(6, 4)) # Standardized Size
...
with tab2:
    ...
    with l:
        ...
        fig_cm, ax_cm = plt.subplots(figsize=(6, 4)) # Standardized Size
    ...
    with r:
        ...
        fig_fi, ax_fi = plt.subplots(figsize=(6, 4)) # Standardized Size
...
with tab3:
    ...
    with st.form("clinical_form"):
        ...
        if st.form_submit_button("Generate Prediction", use_container_width=True):
            ...
            if res == 1: st.error(f"### ⚠️ DIAGNOSIS: HIGH RISK\nConfidence: {prob:.2%}")
            else: st.success(f"### ✅ DIAGNOSIS: LOW RISK\nConfidence: {prob:.2%}")
